...
export default function Header({
  ...
  const { GroCartData, GroWishCount, profile } = React.useContext(AppContext);
    ...
    <View style={[styles.mainContainer]}>
      ...
      <View style={styles.rightIconContainer}>
          ...
          {WishList ? (
            <TouchableOpacity
              style={styles.rightIcons}
              onPress={() => navigation.navigate('GroWishListScreen')}>
          ...
          {Cart ? (
            <TouchableOpacity
              style={styles.rightIcons}
              onPress={() => navigation.navigate('GroCartScreen')}>
              <Text>{showIcon('cart', colours.primaryWhite, windowWidth * (5 / 100))}</Text>
              {Object.keys(GroCartData).length > 0 && (
                <Badge value={Object.keys(GroCartData).length} containerStyle={{ position: 'absolute', top: 5, right: 5, color:colours.primaryWhite}} badgeStyle={{backgroundColor:colours.primaryRed, borderColor: colours.primaryRed}}  />
              )}
            </TouchableOpacity>
          ) : (
            <Text></Text>
